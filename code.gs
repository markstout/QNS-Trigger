...
  const attachmentFolderValue = preferences.attachmentFolder || 'Attachments'; 
...
  const targetAttachmentFolder = getOrCreateFolder(targetDocFolder, attachmentFolderValue); // Attachments folder is inside Notes folder
...
function trigger_makeNoteInex() {
  ...
  const doc = DocumentApp.create('Note Index');
  const body = doc.getBody();

  const header = doc.addHeader();
  header.setText('Note Index');

  const footer = doc.addFooter();
  footer.setText('Generated by Google Apps Script');

  const startTimeText = 'Run started at: ' + startTime.toLocaleString();
  body.appendParagraph(startTimeText);

  sortedTags.forEach(tag => {
    body.appendParagraph(tag).setHeading(DocumentApp.ParagraphHeading.HEADING1);
  ...
  const durationText = 'Run completed in: ' + duration + ' seconds';
  body.insertParagraph(1, durationText);
